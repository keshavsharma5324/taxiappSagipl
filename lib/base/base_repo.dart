...
class BaseRepo {
  ...
  BaseRepo._internal();
  dynamic hitApi(String api,
      {dynamic params, String? accessToken, Options? options, bool? isDelete,int? id,bool? isDirectionApi}) async {
    if (_dio == null) {
      _dio = Dio();}
    ...
    var url = isDirectionApi == null
      ...
      ..options.validateStatus= (status) { return status! < 500; };
        /*..options.validateStatus=(status1) {

            status1 = status;
           return status! < 500;*/
       // };//( status! < 500) as ValidateStatus; //as ValidateStatus;
    //}
    _dio!.interceptors.clear();
    _dio!.interceptors.add(LogInterceptor(
        responseBody: true,
    ...
    if (accessToken != null ) {
     ...
     /* _dio!.interceptors
      ...
      case 203:
      msg = 'error';
      status_code = 203;//e.response!.data["errors"][0]["msg"];
      break;
      case 302:
      // Bearer Token Missing
      msg = 'Invalid Request';
...
class BaseRepo {
  ...
  dynamic getBookingRequest(String token) async {
    try {
      var data = await hitApi(Constants.getBookingRequest.toString(), accessToken: token//, isDelete: false
      );
      return data;
    } catch (e) {
      throw Exception(e.toString());
  ...
  dynamic acceptBookingRequest(params,String token) async {
    try {
      var data = await hitApi(Constants.acceptBookingRequest.toString(),params: params, accessToken: token//, isDelete: false
      );
      return data;
    } catch (e) {
      throw Exception(e.toString());
  ...
  dynamic startRideRequest(params,String token) async {
    try {
      var data = await hitApi(Constants.startRide.toString(),params: params, accessToken: token//, isDelete: false
      );
      return data;
    } catch (e) {
      throw Exception(e.toString());
  ...
  dynamic startEndRequest(params,String token) async {
    try {
      var data = await hitApi(Constants.endRide.toString(),params: params, accessToken: token//, isDelete: false
      );
      return data;
    } catch (e) {
      throw Exception(e.toString());
  ...
  dynamic declineBookingRequest(params,String token) async {
    try {
      var data = await hitApi(Constants.declineBookingRequest.toString(),params: params ,accessToken: token//, isDelete: false
      );
      return data;
    } catch (e) {
      throw Exception(e.toString());
