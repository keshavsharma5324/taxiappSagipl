...
class BookingConfirmScreen extends StatefulWidget {
  ...
  BookingConfirmScreen({this.latitude,this.longitude,this.sourcelatitude,this.sourcelongitude,this.userid,this.driverid,this.accesstoken});

  @override
  State<StatefulWidget> createState() => BookingConfirmScreenState();
}

class BookingConfirmScreenState extends State<BookingConfirmScreen> {
  String? firstname;
  String? lastname;
  String? email;
  String? mobile;
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => BookingConfirmCubit(repo: BaseRepo()),
      child: Scaffold(
        resizeToAvoidBottomInset: false,
        key: _globalKey,
        body: BlocConsumer<BookingConfirmCubit, BookingState>(
            listener: (context, state) {
              ...
              if (state is GetBookingDataState) {
                ...
                bookedride = Ridebooked.fromJson(state.response);
              ...
              children: [
                BookedRoute(bookedride: bookedride,idcar: idcar,idbike: idbike,idauto: idauto,distancebike: distancebike,distanceauto: distanceauto,distancecar: distancecar,accesstoken: widget.accesstoken,driverid: widget.driverid,userid: widget.userid,latitude: widget.latitude,longitude: widget.longitude,sourcelatitude: widget.sourcelatitude,sourcelongitude: widget.sourcelongitude,
                    getbookingconfirmed: () {
                      ...
                      // print('${widget.data['latitude']}  ijfoef3ijf');
                      // NavigationService().navigationKey.currentState.pushNamed(login);
                      context.read<BookingConfirmCubit>().getDriverDetails(/*{
                        //   "email": phone,
                        //  "password": password,
                        //"user_id": widget.,
